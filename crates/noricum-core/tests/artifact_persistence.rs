...
fn test_artifact_store_lifecycle() {
    ...
    store
        .save_c_source("int add(int a, int b) { return a + b; }")
        .unwrap();
    store
        .save_analysis(r#"{"difficulty":"easy","patterns":["arithmetic"]}"#)
        .unwrap();
    store.save_c2rust("unsafe fn add() {}").unwrap();
    store
        .save_translation_final("fn add(a: i32, b: i32) -> i32 { a + b }")
        .unwrap();
    store
        .save_initial_validation(r#"{"compiles":true,"idiomatic_score":90}"#)
        .unwrap();
    store
        .save_final_output("fn add(a: i32, b: i32) -> i32 { a + b }")
        .unwrap();
    store
        .save_manifest(r#"{"name":"add","state":"Validated"}"#)
        .unwrap();
...
fn test_artifact_store_repair_loop_simulation() {
    ...
    store
        .save_translation_final("fn complex() { todo!() }")
        .unwrap();
    store
        .save_initial_validation(r#"{"compiles":false}"#)
        .unwrap();

    // Simulate 3 repair iterations
    store
        .save_repair_iteration(
            1,
            "fn complex() { /* attempt 1 */ }",
            r#"{"compiles":false}"#,
        )
        .unwrap();
    store
        .save_repair_rejected(2, "unsafe fn complex() {}")
        .unwrap();
    store
        .save_repair_iteration(
            3,
            "fn complex() -> i32 { 42 }",
            r#"{"compiles":true,"idiomatic_score":75}"#,
        )
        .unwrap();
    store
        .save_best_version("fn complex() -> i32 { 42 }", 75, 0, true)
        .unwrap();
    store
        .save_final_output("fn complex() -> i32 { 42 }")
        .unwrap();
...
fn test_artifact_store_chunked_translation_simulation() {
    ...
    store
        .save_agreed_signatures("fn parse() -> Result<(), Error>;\nfn lex() -> Vec<Token>;")
        .unwrap();
    store
        .save_foundation("struct Token { kind: TokenKind, span: Span }")
        .unwrap();
    store
        .save_translation_chunk(0, "struct Token { kind: TokenKind, span: Span }")
        .unwrap();
    store
        .save_translation_chunk(1, "fn lex() -> Vec<Token> { vec![] }")
        .unwrap();
    store
        .save_translation_chunk(2, "fn parse() -> Result<(), Error> { Ok(()) }")
        .unwrap();
...
fn test_artifact_store_modular_migration_simulation() {
    ...
    store
        .save_translation_module("parser", "fn parse() {}")
        .unwrap();
    store
        .save_translation_module("lexer", "fn lex() {}")
        .unwrap();
    store
        .save_translation_module("eval", "fn eval() {}")
        .unwrap();
...
fn test_artifact_store_retranslation_artifacts() {
    ...
    store
        .save_retranslation("stub", "fn foo() { /* retranslated from stub */ }")
        .unwrap();
    store
        .save_retranslation("unsafe", "fn bar() { /* retranslated to remove unsafe */ }")
        .unwrap();
    store
        .save_retranslation_stall("fn baz() { /* stall retranslation */ }")
        .unwrap();
